...
#ifndef _my_spiffs_editor_h
#define _my_spiffs_editor_h
...
void setupWebserver(){
  ...
  if (http_pass!="") {
    ArduinoOTA.setPassword(http_pass);
    DEBUG_PRINTLN("OTA password set " + String(http_pass));
  }
  ...
  MDNS.begin(strHostname.c_str());
  DEBUG_PRINT("Open http://");
  DEBUG_PRINT(strHostname);
  DEBUG_PRINTLN(".local/edit to see the file browser");
  ...
  String strIPAddr = String(local[0]) + "." + String(local[1]) + "." + String(local[2]) + "." + String(local[3]);
  server.on("/settings", HTTP_GET, [strIPAddr](AsyncWebServerRequest *request){
    AsyncWebParameter* pBrightness = request->getParam("brightness");
    if (pBrightness) {
      String data = pBrightness->value();
      String mqttTopic = String(strTopicPrefixChipID + "brightness");
      String mqttPayload = data;
      mqttMessages.push(sMqttMessage(mqttTopic,mqttPayload));
      //publishMQTTMessage(mqttTopic,mqttPayload,true);
      DEBUG_PRINT("GET '/brightness': ");
      DEBUG_PRINTLN(data);
    }
    AsyncWebParameter* pColor = request->getParam("color");
    if (pColor) {
      String data = pColor->value();
      String mqttTopic = String(strTopicPrefixChipID + "color");
      String mqttPayload = data;
      mqttMessages.push(sMqttMessage(mqttTopic,mqttPayload));
      //publishMQTTMessage(mqttTopic,mqttPayload,true);
      DEBUG_PRINT("GET '/color': ");
      DEBUG_PRINTLN(data);
    }
    AsyncWebParameter* pColorEffect = request->getParam("color_effect");
    if (pColorEffect) {
      String data = pColorEffect->value();
      String mqttTopic = String(strTopicPrefixChipID + "color/effect");
      String mqttPayload = data;
      mqttMessages.push(sMqttMessage(mqttTopic,mqttPayload));
      //publishMQTTMessage(mqttTopic,mqttPayload,true);
      DEBUG_PRINT("GET '/color_effect': ");
      DEBUG_PRINTLN(data);
    }
    AsyncWebParameter* pBackground = request->getParam("background");
    if (pBackground) {
      String data = pBackground->value();
      String mqttTopic = String(strTopicPrefixChipID + "background");
      String mqttPayload = data;
      mqttMessages.push(sMqttMessage(mqttTopic,mqttPayload));
      //publishMQTTMessage(mqttTopic,mqttPayload,true);
      DEBUG_PRINT("GET '/background': ");
      DEBUG_PRINTLN(data);
    }
    AsyncWebParameter* pEffect = request->getParam("mode");
    if (pEffect) {
      String data = pEffect->value();
      String mqttTopic = String(strTopicPrefixChipID + "mode");
      String mqttPayload = data;
      mqttMessages.push(sMqttMessage(mqttTopic,mqttPayload));
      //publishMQTTMessage(mqttTopic,mqttPayload,true);
      DEBUG_PRINT("GET '/mode': ");
      DEBUG_PRINTLN(data);
    }
    ...
    String json = "{";
          ...
          json += "\"color\":\""+String(LedR)+","+String(LedG)+","+String(LedB)+"\",";
          json += "\"color_effect\":\""+colorEffect+"\",";
          json += "\"background\":\""+background+"\",";
          json += "\"mode\":\""+effectMode+"\"";
          json += "}";
    request->send(200, "text/plain", json);
    DEBUG_PRINT("GET '/settings' response: ");
    DEBUG_PRINTLN(json);
  });

  server.on("/brightness", HTTP_GET, [](AsyncWebServerRequest *request){
    AsyncWebParameter* pPayload = request->getParam("payload");
    String data = pPayload->value();
    String mqttTopic = String(strTopicPrefixChipID + "brightness");
    String mqttPayload = data;
    DEBUG_PRINT("GET '/brightness': ");
    DEBUG_PRINTLN(data);
  ...
  server.on("/color_effect", HTTP_GET, [](AsyncWebServerRequest *request){
    AsyncWebParameter* pPayload = request->getParam("payload");
    String data = pPayload->value();
    String mqttTopic = String(strTopicPrefixChipID + "color/effect");
    String mqttPayload = data;
  ...
  server.on("/tetris", HTTP_GET, [](AsyncWebServerRequest *request){
    AsyncWebParameter* pPayload = request->getParam("payload");
    String data = pPayload->value();
    String mqttTopic = String(strTopicPrefixChipID + "tetris");
    String mqttPayload = data;
    DEBUG_PRINT("GET '/tetris': ");
    DEBUG_PRINTLN(data);
    request->send(200, "text/plain", "ok");
    mqttMessages.push(sMqttMessage(mqttTopic,mqttPayload));
    //publishMQTTMessage(mqttTopic,mqttPayload,false);
  });
  server.on("/tetris_move", HTTP_GET, [](AsyncWebServerRequest *request){
    AsyncWebParameter* pPayload = request->getParam("payload");
    String data = pPayload->value();
    String mqttTopic = String(strTopicPrefixChipID + "tetris/move");
    String mqttPayload = data;
    DEBUG_PRINT("GET '/tetris_move': ");
    DEBUG_PRINTLN(data);
    request->send(200, "text/plain", "ok");
    mqttMessages.push(sMqttMessage(mqttTopic,mqttPayload));
    //publishMQTTMessage(mqttTopic,mqttPayload,false);
  ...
  server.onNotFound([](AsyncWebServerRequest *request){
    ...
    if(request->method() == HTTP_GET)
      ...
      DEBUG_PRINT("UNKNOWN");
    DEBUG_PRINT(" http://");
    DEBUG_PRINT(request->host());
    DEBUG_PRINTLN(request->url());
    if(request->contentLength()){
      DEBUG_PRINT("_CONTENT_TYPE: ");
      DEBUG_PRINTLN(request->contentType());
      DEBUG_PRINT("_CONTENT_LENGTH: ");
      DEBUG_PRINTLN(request->contentLength());
    }
  ...
  server.onFileUpload([](AsyncWebServerRequest *request, String filename, size_t index, uint8_t *data, size_t len, bool final){
    if(!index){
      DEBUG_PRINT("UploadStart: ");
      DEBUG_PRINTLN(filename.c_str());
    }
    DEBUG_PRINT((const char*)data);
    if(final){
      DEBUG_PRINT("UploadEnd: ");
      DEBUG_PRINT(filename);
      DEBUG_PRINT("(");
      DEBUG_PRINT(index+len);
      DEBUG_PRINTLN(")");
    }
  });
  server.onRequestBody([](AsyncWebServerRequest *request, uint8_t *data, size_t len, size_t index, size_t total){
    if(!index){
      DEBUG_PRINT("BodyStart: ");
      DEBUG_PRINTLN(total);
    }
    DEBUG_PRINT((const char*)data);
    if(index + len == total){
